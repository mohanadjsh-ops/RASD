  ...
  return (
    <section>
      <div className="flex items-center justify-between gap-3">
        ...
        {isAdmin ? <button className="rounded-md bg-electric px-4 py-2 text-sm font-semibold text-white">Add source</button> : null}
      </div>
      <div className="mt-5 overflow-hidden rounded-md border border-line bg-panel">
        {sources.map((source) => (
          <div key={source.id} className="grid gap-3 border-b border-line p-4 last:border-0 md:grid-cols-[1fr_auto_auto]">
            ...
            <span className="rounded-md border border-line px-2 py-1 text-xs text-slate-300">{source.source_type}</span>
            <span className={source.enabled ? "text-verified" : "text-slate-500"}>{source.enabled ? "enabled" : "disabled"}</span>
          </div>
        ))}
      </div>
    </section>
